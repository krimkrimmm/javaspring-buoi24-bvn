...
import vn.scrip.buoi24.exception.BadRequestException;
import vn.scrip.buoi24.exception.NotFoundException;

import vn.scrip.buoi24.model.request.CreateReviewRequest;
import vn.scrip.buoi24.model.request.UpdateReviewRequest;
...
public class ReviewService {
    ...
    public Review updateReview(Integer id, UpdateReviewRequest request) {
        ...
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("Không tìm thấy user có id = " + userId));
        Review review = reviewRepository.findById(id).orElseThrow(() -> new NotFoundException("Không tìm thấy review có id = " + id));
